...
// API配置 - 需要用户填入自己的密钥
const API_CONFIG = {
    APP_KEY: '1ae292ef39471601', // 请填入您的应用ID
    APP_SECRET: 'k3wCpLujOOGx8nrTJEPiVPhQMO20HiQS', // 请填入您的应用密钥
    API_URL: 'https://openapi.youdao.com/v2/correct_writing_image'
};
...
function initializeApp() {
    ...
    if (!API_CONFIG.APP_KEY || !API_CONFIG.APP_SECRET) {
        showMessage('请先在script.js文件中配置您的API密钥', 'error');
    }
...
function calculateSign(appKey, appSecret, q, salt, curtime) {
    ...
    return CryptoJS.SHA256(strSrc).toString();
}
...
async function submitImageEssay(title, grade, imageData) {
    ...
    addAuthParams(API_CONFIG.APP_KEY, API_CONFIG.APP_SECRET, params);
